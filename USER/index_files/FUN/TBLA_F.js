function Plusove_Minuse(n) {
  for (i = 0; i < PLUS_MINUS.length; i++) {
    if (i != n) {
      PLUS_MINUS[i].innerHTML = "+";
    }
  }

  if (n != null) {
    if (PLUS_MINUS[n].innerHTML == "+") {
      PLUS_MINUS[n].innerHTML = "-";
    } else {
      PLUS_MINUS[n].innerHTML = "+";
    }
  }
...
$("#Service_btn").click(function () {
  $("#Local_btn").slideToggle(600, "linear");
  Plusove_Minuse(null);
  ...
  $("#SWITCH_GG").slideToggle("fast");
  $("#LOG_OUT").slideToggle("fast");
...
function TuriMAPOT() {
  ...
  let G_2 = "&amp;z=11'";
  let JJ = document.getElementById("GGL_URL").value;
  let YY = ZemiOKdel(JJ);
...
function ZemiOKdel(a) {
  let O = { ADR: null, KOD: null };
  let NW = a.split("/place/", 2);
  let NNWW = NW[1].split("/data="); ///"Aalen,+Germany/@48.8236068,10.0546206,12z"

  let OO_1 = NNWW[0].split("/@");
  let O_1 = "q=" + OO_1[0];

  let QQ_1 = OO_1[1].split(",");
  let Q_1 = "ll=" + QQ_1[0] + QQ_1[1];
